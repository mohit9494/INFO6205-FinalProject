...
public class MSDStringSortTest {
    ...
    public void sortChinese0(){
        ...
        new MSDStringSort(expected.length,ThreeWayRadixQuicksort.getPinyinCharOperator())
    ...
    public void sortTelugu0() {
        ...
        String[] input = Arrays.copyOf(expected,expected.length);
        Collections.shuffle(Arrays.asList(input));
        new MSDStringSort(expected.length,ThreeWayRadixQuicksort.getStringCharOperator())
                .withInsertionSortMSDOperator(InsertionSortMSD.getStringBinaryOperator())
                .sort(input);
        assertArrayEquals(expected, input);
    }
